...
export const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const result = await login(authDetail)
  ...
  const handleLoginGuest = async () => {
    emailRef.current.value = "pascalmartin973@gmail.com"
    passwordRef.current.value = "sjrkrjqoseugjfdlrh!"
    try {
      const authDetail = {
        email: emailRef.current.value,
        password: passwordRef.current.value
  ...
  return (
    <main>
      ...
      <button className="mt-3 cursor-pointer text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-3 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={handleLoginGuest}>Login As Guest</button>
    </main>
  )
}
